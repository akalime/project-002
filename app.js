...
window.P002App = (() => {
  ...
  async function startReading() {
    ...
    const _cur = window._currentCourse;
    const _curIdx = _cur?.sections?.findIndex(s => s.section_number === sectionData?.meta?.section);
    const nextMeta = _cur?.sections?.[_curIdx + 1] ?? null;
    const nextTitle = document.getElementById('stickyNextTitle');
    if (nextTitle) nextTitle.textContent = nextMeta?.title || 'Course complete';
  ...
  function showKcResults() {
    ...
    const _cc = window._currentCourse;
    const _ci = _cc?.sections?.findIndex(s => s.section_number === sectionData?.meta?.section);
    const hasNext = _cc?.sections && _ci !== undefined && _ci < _cc.sections.length - 1;
  ...
  async function importBook(book) {
    ...
    try {
      ...
      // SSE generation — runs server-side, user can navigate freely
      showToast('&#128218; Generating "' + book.title.slice(0, 25) + (book.title.length > 25 ? '...' : '') + '" — you can keep browsing!', true);
      ...
      P002Api.generateCourse(
        ...
        (pct, sectionTitle, done) => {
          ...
          const b = document.getElementById('import-' + safeId);
          if (b) {
            b.innerHTML = done ? '&#10003; Added' : pct + '%';
            if (done) { b.classList.remove('importing'); b.classList.add('imported'); }
          }
          // Refresh home screen if user is there
          if (done) {
            showToast('&#9989; "' + book.title.slice(0, 25) + (book.title.length > 25 ? '...' : '') + '" is ready!', true);
        ...
        (courseId, outline) => {
          // Outline done — course card appears on home screen immediately
          loadMyCourses();
        }
      ).catch(e => {
        const b = document.getElementById('import-' + safeId);
        if (b) { b.classList.remove('importing'); b.innerHTML = '+ Import'; }
